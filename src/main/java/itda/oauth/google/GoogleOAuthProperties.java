...
public record GoogleOAuthProperties(
    ...
    void validateWhenEnabled() {
        ...
        List<String> allowlist = allowedCallbackUrls.stream()
        ...
        for (String callback : allowlist) {
            validateCallback(callback, "allowed-callback-urls");
        }
        if (transactionTtl == null || transactionTtl.toMillis() <= 0) {
            throw invalid("transaction-ttl must be at least one millisecond");
        }
    ...
    public URI backendRedirectUri() {
        validateBackendRedirect(redirectUri);
        return URI.create(redirectUri);
    }

    public URI successCallback() {
        validateConfiguredCallback(successCallbackUri);
        return URI.create(successCallbackUri);
    }

    public URI errorCallback() {
        validateConfiguredCallback(errorCallbackUri);
        return URI.create(errorCallbackUri);
    }

    private void validateConfiguredCallback(String value) {
        validateCallback(value, "callback URI");
        if (allowedCallbackUrls == null || !allowedCallbackUrls.contains(value)) {
            throw invalid("callback URI is not allowlisted");
        }
    }

    private static void validateBackendRedirect(String value) {
        URI uri = parse(value, "redirect-uri");
        if (!isSecureOrExplicitLocalhost(uri)) {
            throw invalid("redirect-uri must use HTTPS, except explicit localhost HTTP");
        }
        if (!"/login/oauth2/code/google".equals(uri.getPath())) {
            throw invalid("redirect-uri must target /login/oauth2/code/google");
        }
    }
    ...
    private static void validateCallback(String value, String name) {
        ...
        if (!isSecureOrExplicitLocalhost(uri)) {
            throw invalid(name + " must use HTTPS, except explicit localhost HTTP");
        }
    }
    ...
    private static URI parse(String value, String name) {
        ...
        try {
            ...
            throw invalid(name + " is not a valid URI");
        }
    }
    ...
    private static void requireText(String value, String name) {
        if (value == null || value.isBlank()) {
            throw invalid(name + " is required when Google OAuth is enabled");
        }
    }

    private static IllegalStateException invalid(String message) {
        return new IllegalStateException("Invalid Google OAuth configuration: " + message);
    }
}
