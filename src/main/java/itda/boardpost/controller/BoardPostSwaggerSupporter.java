...
public interface BoardPostSwaggerSupporter {
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "201", description = "게시글 작성 성공", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
            examples = @ExampleObject("""
                    {"success":true,"message":"게시글이 등록되었습니다.","data":{"postId":1,"boardId":1,"authorPet":{"petId":2,"publicTag":"dog-2","nickname":"두부","profileUrl":null,"verified":false},"title":"산책 친구 구해요","content":"오늘 저녁 같이 산책해요.","images":[{"mediaId":10,"url":"https://...","displayOrder":0}],"reactionCount":0,"reactedByMe":false,"helpfulCount":0,"helpfulByMe":false,"version":0,"createdAt":"2026-08-10T00:00:00Z","updatedAt":"2026-08-10T00:00:00Z"},"error":null}
                    """)))
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "게시글 피드 조회 성공", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
            examples = @ExampleObject("""
                    {"success":true,"message":"게시글 목록이 조회되었습니다.","data":{"items":[{"postId":1,"boardId":1,"authorPet":{"petId":2,"publicTag":"dog-2","nickname":"두부","profileUrl":null,"verified":false},"title":"산책 친구 구해요","content":"오늘 저녁 같이 산책해요.","images":[],"reactionCount":4,"reactedByMe":true,"helpfulCount":2,"helpfulByMe":false,"version":0,"createdAt":"2026-08-10T00:00:00Z","updatedAt":"2026-08-10T00:00:00Z"}],"page":{"nextCursor":"MjAyNi0wOC0xMFQwMDowMDowMFp8MQ","hasNext":true}},"error":null}
                    """)))
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "게시글 상세 조회 성공", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
            examples = @ExampleObject("""
                    {"success":true,"message":"게시글이 조회되었습니다.","data":{"postId":1,"boardId":1,"authorPet":{"petId":2,"publicTag":"dog-2","nickname":"두부","profileUrl":null,"verified":false},"title":"산책 친구 구해요","content":"오늘 저녁 같이 산책해요.","images":[],"reactionCount":4,"reactedByMe":true,"helpfulCount":2,"helpfulByMe":false,"version":0,"createdAt":"2026-08-10T00:00:00Z","updatedAt":"2026-08-10T00:00:00Z"},"error":null}
                    """)))
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "반응 추가 성공", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
            examples = @ExampleObject("""
                    {"success":true,"message":"게시글 반응 상태가 변경되었습니다.","data":{"postId":101,"type":"HELPFUL","reacted":true,"reactionCount":24},"error":null}
                    """)))
    ...
    ResponseEntity<ApiResponse<BoardPostReactionResponse>> addReaction(
            ...
            @Parameter(description = "반응 타입 (LIKE, HELPFUL)", example = "HELPFUL") BoardPostReactionType type
    );

    @Operation(summary = "게시글 반응 취소", description = "현재 활성 반려견의 LIKE 또는 HELPFUL 반응을 멱등하게 취소합니다. 요청 본문은 없습니다. 비공개·차단 게시글은 404로 은닉됩니다. BoardPostResponse의 reactionCount/reactedByMe는 LIKE 전용인 반면, 이 BoardPostReactionResponse의 reactionCount는 요청한 type의 현재 count입니다.")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "반응 취소 성공", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
            examples = @ExampleObject("""
                    {"success":true,"message":"게시글 반응 상태가 변경되었습니다.","data":{"postId":101,"type":"HELPFUL","reacted":false,"reactionCount":23},"error":null}
                    """)))
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "지원하지 않는 반응 타입 (VALIDATION_FAILED)")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "인증 실패 (UNAUTHORIZED)")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "활성 반려견 필요 또는 자기 게시글 반응 금지 (ACTIVE_PET_REQUIRED, BOARD_POST_SELF_REACTION_FORBIDDEN)")
