...
**채팅방 수명주기 배치(무응답 인사 방 정리·30일 보관)는 M2 신규 작업이 아니다.** 이 기능은 `04_M1_마일스톤_WBS.md`의 `M1-038`이며, PR #50(2026-08-06 머지)이 M1 종료 기한을 넘겨 뒤늦게 완료했다. `M1-038`의 상태는 아직 그 문서에 "미착수"로 남아 있어 **M1 WBS 갱신이 별도로 필요하다** — 이 문서가 대신 갱신하지 않는다. 다만 이 배치가 WebSocket SEND·이벤트 동작과 직접 상호작용하므로(`06_M2_WebSocket_계약.md` §4·§5·§10 참고) §4 각주와 §7 위험 항목에서 참고로만 언급한다.
...
**React Native는 M2에 포함하지 않는다(2026-08-07 확정).** `04_M1_마일스톤_WBS.md`의 마지막 문단("M2·M3 상세 WBS는 본 문서에 넣지 않는다")이 후속 항목으로 React Native를 예고하고 있으나 이는 낡은 서술이다. `00_최신_제품정책.md` §1의 8개 항목이 M2 범위의 정본이며, 해당 문장은 M1 WBS 정정 PR에서 함께 지운다(§8).
...
- `04_M1_마일스톤_WBS.md` 정정 PR을 별도로 올린다. 두 건이다 — `M1-038` 상태를 "완료"로, 마지막 문단의 M2 후속 항목에서 React Native 삭제.
