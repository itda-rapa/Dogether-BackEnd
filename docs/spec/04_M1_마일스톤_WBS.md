...
| M1-039 | 7/26~7/28 | 차단 | BE-2 | BE-3 검토 | M1-006, M1-028 | User 단위 차단 API·검사·UI, 관계 정리, 콘텐츠 숨김, 해제 없음 | P0 | 완료(`BlockApiContractPostgreSqlIntegrationTest`가 차단 API를, `BlockIntegrationTest`의 PostgreSQL nested suites가 양방향 판정·중복 요청 단일 행·정리 실패 rollback을, `BlockServiceTest`가 관계 정리를, `ChatRestPollingApiPostgreSqlIntegrationTest` 35가 차단 후 양방향 방 목록·상세·이력·전송 차단을 검증. OpenAPI와 `BlockController`에는 M1 차단 해제 endpoint가 없다. 2026-08-12 최신 dev XML 모두 0 실패·0 오류) |
